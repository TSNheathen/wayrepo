...
| --- | --- |
| Static host, entry-point, route, and feature/contract shapes from the current production package | Authenticated domain/channel/module/role configuration and feature/license export |
| Deployed ABRA-shaped connector directions, windows, batches, and manual recovery seams | Which external business systems are active; their roles; active connector generations, schedules, outcomes, backlog, ownership, and reconciliation evidence |
...
The current production package and public surface are sufficient to seed a runtime-state evidence request, but insufficient to declare the replacement acceptance boundary. The package contains no trustworthy live database or runtime-configuration export (`docs/research/akros-production-baseline.md:5,23-25`). Its own ignore rules exclude runtime-bearing directories such as logs, local configuration, data, migration state, caches, transport, backups, certificates, and sessions (`../context/team-1633516729428-akros-124b361ce570/.gitignore:7-31`).

The missing evidence is not merely infrastructure detail. It includes the live domain/channel/module matrix, actual payment and shipping rules, the identities and roles of active external business systems, authoritative connector paths and schedules, aggregate data volumes and shapes, operator workflows, URL/SEO inventory, and active externally consumed interfaces. These facts determine what the new architecture must preserve, change, archive, or retire. The full gap is corroborated by the existing baseline (`docs/research/akros-production-baseline.md:230-243`).

The deployed ABRA-shaped source is a particularly useful seed, but cannot substitute for runtime discovery. It declares imports for products, stock, category membership, prices, parameters, categories, recent orders, customers, and invoices; order export; and customer export as part of order handling (`../context/team-1633516729428-engine-a6883ea926b4/wrshop/Module/Is/AbraApi/BaseControl.php:27-54,99-220,228-232`). It also contains numeric cadence-like declarations, a one-day recent-order window, explicit product/customer page sizes, a ten-order export batch, duplicate lookup, a 31-day invoice-search window, and manual re-export. Those are deployed implementation clues, not proof of the connected product, its operational role, an active scheduler, or production outcomes (`../context/team-1633516729428-engine-a6883ea926b4/wrshop/Module/Is/AbraApi/BaseControl.php:27-54,115-120,179-196`; `../context/team-1633516729428-engine-a6883ea926b4/wrshop/Module/Is/AbraApi/Control/OrderControl.php:65-97,168-186,280-290`; `../context/team-1633516729428-akros-124b361ce570/project/custom/recreate.php:1-5`). Coexisting `AbraApi`, `AbraG3`, and legacy adapters make active-system and active-path confirmation mandatory (`docs/research/akros-production-baseline.md:164-174`).
...
- [ ] Name a business owner, Akros operator, database owner, owner for each discovered External Business System, payment owner, shipping owner, and SEO/analytics owner for the capture.
...
- [ ] Do not invoke manual connector routes, old integration/debug endpoints, payment returns, carrier callbacks, or marketplace APIs. Their existence is contract evidence only; the deployed ABRA-shaped manual route can initiate work (`../context/team-1633516729428-akros-124b361ce570/project/custom/recreate.php:1-5`).
...
- [ ] Never capture PAN, CVV, payment tokens, gateway transaction identifiers, bank details, External Business System credentials, invoice PDFs, claim attachments, free-text order/claim notes, search terms, or raw URLs with query strings.
...
The production package proves configured host candidates, including B2C/B2B-shaped hosts, but not the database-controlled access rules or enabled feature set (`../context/team-1633516729428-akros-124b361ce570/conf.d/nginx.conf:1-8`; `docs/research/akros-production-baseline.md:103-119,230-236`).
...
### 3. External-business-system contracts, paths, and schedules

The deployed ABRA-shaped connector supplies a discovery seed for catalog, stock, category membership, prices, parameters, customers, orders/statuses, invoices, customer/order export, and authorized invoice access (`docs/research/akros-production-baseline.md:148-162`). Customer matching reasons about firm, office, address, and contact rather than email alone (`../context/team-1633516729428-engine-a6883ea926b4/wrshop/Module/Is/AbraApi/Control/CustomerControl.php:31-113`). Runtime evidence must first establish which External Business Systems actually perform these roles.

- [ ] Identify every active External Business System by product, system type, business role, owner, and evidence of activity. Do not infer that a system is an ERP from the data it exchanges.
- [ ] Identify the **active Connector generation and entry point per entity**: `AbraApi`, `AbraG3`, another legacy path, a different product's connector, manual-only, or inactive. Do not record endpoint credentials.
...
- [ ] Record order-export duplicate detection, customer/office matching, status mapping, invoice-link/PDF behavior, retry/replay, backfill, reconciliation, manual recovery, and alert ownership. The deployed ABRA-shaped path limits automatic candidates to ten and writes external/error state locally; current production behavior must be observed rather than assumed (`../context/team-1633516729428-engine-a6883ea926b4/wrshop/Module/Is/AbraApi/Control/OrderControl.php:19-62,65-97,139-186,245-290`).
- [ ] Produce schema-only contracts with synthetic examples for every retained entity and callback. Do not export customer, address, order, invoice, or External Business System payload samples.

**Output:** `External Business System registry`, `entity-contract matrix`, `schedule/run aggregates`, `mapping-health aggregates`, and `operator recovery walkthroughs`.
...
The old CMS surface indicates possible operational areas, while the actual roles, handoffs, overrides, and post-Connector edits are absent (`docs/research/akros-production-baseline.md:121-129,230-243`).
...
- [ ] Walk through price/discount/stock investigation and document which fields can be overridden locally versus owned by each External Business System.
...
- [ ] Walk through order search/detail, status intervention, external-system export/re-export decisions, invoice retrieval, shipment/tracking exception, cancellation/refund/credit handling, and claim processing.
- [ ] Walk through payment/carrier reconciliation, failed Connector run/backlog recovery, feed/marketplace failure, and content/URL correction.
...
Candidate interfaces include the deployed ABRA-shaped connectors, payment and carrier integrations, ARES/VIES, marketplace/comparison feeds and APIs, analytics/marketing tags, and invoice retrieval. Presence in deployed code is only a discovery list; the actual External Business Systems and interface activity must be proven by sanitized configuration plus aggregate call evidence (`docs/research/akros-production-baseline.md:146-189`).
...
- [ ] Include public storefront/service APIs, every discovered External Business System, payment returns/webhooks, carrier/pickup APIs, ARES/VIES, Heureka/Zboží and other feeds/marketplaces, analytics/conversion/consent tags, email delivery, invoice access, and any B2B machine consumer discovered at runtime.
...
This capture turns the deployed capability inventory into an evidence-backed replacement boundary. It will let the Wayfinder map decide the live B2C/B2B scope, Systems of Record, surviving External Business System contracts and Connectors, payment/shipping semantics, migration/retention sizing, operator acceptance surface, URL/SEO continuity, and externally coordinated cutover without expanding into legacy-platform remediation. Those are the decision gaps already identified by the baseline (`docs/research/akros-production-baseline.md:245-257`).

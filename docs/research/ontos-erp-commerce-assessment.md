...
- Current runtime documentation explicitly leaves concrete business Actions, access-management Actions, external queues, full manifest generation, and generic exact-response replay out of scope (`../context/ontos/app/packages/core-runtime/docs/action-ecosystem.md:80-87`). [OntOS #97](https://github.com/TechsioCZ/ontos/issues/97) confirms that generic replay is intentionally not a Core responsibility.
- An already-successful idempotent Action returns an already-committed/replay-unavailable outcome rather than the original response (`../context/ontos/app/packages/core-runtime/src/core-sdk.ts:711-735`). Checkout, capture, refund, and fulfillment interfaces must therefore return stable references or reconstruct semantic responses from owner canonical state; a bounded exact acknowledgement belongs to an explicit Business Module or Connector contract, not generic Core hardening.
...
| CoreSDK Action/read envelope | Both products benefit from consistent authz, policy, effect deduplication, audit and transaction semantics | Commerce workflows must add owner-defined semantic reconstruction or bounded acknowledgement replay where required, plus locking, compensation and reconciliation |

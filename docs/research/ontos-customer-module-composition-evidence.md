...
Research support for **Decide the shared Ontos kernel contract**. This note verifies the customer/module correction against first-party local architecture documents, the current OntOS `app/`, the Akros deployment snapshot, and the legacy WRShop engine. It deliberately does not inspect `context/ontos/mvp/` or `context/ontos/mvp2/`.
...
The current Akros glossary says Akros is a sibling of OntOS, Core is a shared foundation consumed by both products, and Akros Commerce Modules are not OntOS Business Modules (`wayrepo/CONTEXT.md:9-23`). The accepted commerce-foundation decision repeats that shape (`wayrepo/docs/product/akros-commerce-foundation.md:5-27`). Those statements directly contradict both the canonical OntOS sources and the newly confirmed product direction. They should be superseded, not treated as constraints on this ticket.
...
| Party / Counterparty | OntOS already defines Party and Counterparty across suppliers, corporate buyers, wholesalers, tenants, guests, and external managers (`origin/develop:docs/CONTEXT.md:107-121`; `origin/develop:docs/02_GLOSSARY.md:103-125`). The accepted Akros integration decision assigns Party identity and Counterparty relationships to a Party Registry (`wayrepo/docs/product/akros-data-authority-integration-contracts.md:13-24`). | A Party Registry should be an OntOS Foundational Module, not an Akros service and not Core. Core retains Principal/authentication/authorization context; modules own role-specific profiles and policies. |
...
| Integrations | Core owns transactional outbox/worker foundations; provider-specific export shape remains a module/integration responsibility (`origin/develop:docs/06_CORE_KERNEL.md:21-33`; `origin/develop:docs/04_C4_MODEL.md:23-25,43-45`). Akros and N1 demonstrate different ABRA/Pohoda mappings over shared commerce behavior. | Keep durable dispatch, identity, audit, and worker mechanics in Core. Put provider-neutral business handoff contracts in the owning Business Module and provider/customer mappings in replaceable Connectors. A shared Integration Operations surface may expose backlog/replay/reconciliation without owning imported facts. |
...
6. **Reconcile source contradictions explicitly.** The older OntOS pack puts e-shop outside OntOS and describes one jointly deployable modular monolith (`origin/develop:docs/03_ARCHITECTURE_OVERVIEW.md:15-19,41-43`), while the authoritative app-local architecture mandates independently deployable modules and the newly confirmed product direction puts commerce modules inside OntOS (`origin/develop:app/docs/architecture/MODULE_MANIFESTS.md:125-131`). Update the root canonical docs rather than preserving both claims.

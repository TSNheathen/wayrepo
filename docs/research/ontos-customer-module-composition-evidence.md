...
- Deployment identity (`appId`) and business-capability identity (`moduleId`) are distinct. The current V0 implementation supports one `business_module` per MicroVertical deployment, while installation and per-tenant activation are separate (`origin/develop:app/docs/architecture/MODULE_MANIFESTS.md:1-15,17-34`). [OntOS #92](https://github.com/TechsioCZ/ontos/issues/92) confirms the target extension: the same one-module delivery/catalog path accepts an explicit `foundational_module` kind and evaluates versioned composition closure without moving Foundational Module meaning into Core.
...
CRM currently names its aggregates `Customer` and `Contact`, whereas the root OntOS vocabulary distinguishes Party, Counterparty, Principal, and Legal Entity. CRM therefore proves the owner-local Business Module/data seam, not that `Customer` is the final cross-module identity model. [Define Party Registry, CRM Customer, and commerce Counterparty ownership](https://github.com/TechsioCZ/ontos/issues/95) confirms that the non-live CRM model may be replaced without compatibility or backfill: Party Registry owns shared identity and relationships, while later CRM modules own engagement profiles and workflows.
...
**What it does not prove:** independent deployability does not require every module to run on a separate host at all times, and it does not prove the desired failure behavior has passed tests. Nor does it decide whether customer deployments and data planes are isolated per customer or shared across tenants. Graceful module-failure behavior and customer deployment topology require separate decisions and proof. At pinned commit `17ba8f8`, the older architecture pack still called the system a jointly deployable modular monolith; the authoritative app-local contract explicitly superseded that implementation rule. [OntOS #93: Record the independently deployable MicroVertical architecture and retire stale modular-monolith guidance](https://github.com/TechsioCZ/ontos/issues/93) subsequently confirmed the invariant, and [OntOS ADR-0016](https://github.com/TechsioCZ/ontos/blob/develop/docs/adr/0016-independently-deployable-microverticals.md) records its trade-offs, consequences, migration impact, implemented mechanisms, and remaining production proof.
...
| Party / Counterparty | OntOS already defines Party and Counterparty across suppliers, corporate buyers, wholesalers, tenants, guests, and external managers (`origin/develop:docs/CONTEXT.md:107-121`; `origin/develop:docs/02_GLOSSARY.md:103-125`). [Define Party Registry, CRM Customer, and commerce Counterparty ownership](https://github.com/TechsioCZ/ontos/issues/95) confirms Party Registry as their tenant-scoped identity boundary. | Party Registry is a Foundational Module, not an Akros service and not Core. It owns sparse/Unresolved identity, Contact Points, Party Relationships, Counterparty relationships/roles, matching, correction, and merge. Core retains Principal authentication/authorization; CRM and Commerce own role-specific profiles and workflows; provider correlations remain Connector Registry facts. |
